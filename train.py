...
def train():
    ...
    for step in pbar:
        model.train()
        images, input_ids, attention_mask = next(train_iter)
        ...
        if step % config.eval_every == 0 or step == config.total_steps:
            ...
            avg_recall = (metrics["i2t_r1"] + metrics["t2i_r1"]) / 2
            print(
                f"\n[Step {step}] I2T R@1: {metrics['i2t_r1']*100:.1f}% | "
                f"T2I R@1: {metrics['t2i_r1']*100:.1f}% | "
                f"Avg R@1: {avg_recall*100:.1f}%"
            )
